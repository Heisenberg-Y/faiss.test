...
GetResultHitCount(const faiss::Index::idx_t* ground_index, const faiss::Index::idx_t* index, size_t ground_k, size_t k,
    ...
    for (size_t i = 0; i < nq; i++) {
        ...
        for (size_t j_c = 0; j_c < k; j_c++) {
            int r_c = index[i * k + j_c];
            for (size_t j_g = 0; j_g < min_k / index_add_loops; j_g++) {
...
#ifdef CUSTOMIZATION
        auto cpu_ivf_index = dynamic_cast<faiss::IndexIVF*>(cpu_index);
    ...
    if (query_mode != MODE_CPU) {
        ...
        switch (query_mode) {
            ...
            case MODE_MIX: {
                ...
                delete gpu_index;
                delete index_composition.quantizer;
                index_composition.quantizer = nullptr;
            ...
            case MODE_GPU:
#ifdef CUSTOMIZATION
                if(index_key.find("Hybrid") == std::string::npos){
                    index_composition.mode = 0;
                    ...
                    gpu_index = faiss::gpu::index_cpu_to_gpu(&res, GPU_DEVICE_IDX, &index_composition, &option);
                    copy_time = elapsed() - copy_time;
                    printf("[%.3f s] Copy all completed, cost %f s\n", elapsed() - t0, copy_time);
                }else {
                    index_composition.mode = 1;  // 0: all data, 1: copy quantizer, 2: copy data
                    ...
                    delete gpu_index;
                    copy_time = elapsed() - copy_time;
                    printf("copy quantizer time: %f s\n", copy_time);

                    index_composition.mode = 2;
                    copy_time = elapsed();
                    gpu_index = faiss::gpu::index_cpu_to_gpu(&res, GPU_DEVICE_IDX, &index_composition, &option);
                    copy_time = elapsed() - copy_time;
                    printf("copy data time: %f s\n", copy_time);
                }
#else
                // warm up the transmission
...
test_ann_hdf5(const std::string& ann_test_name, const std::string& cluster_type, const std::string& index_type,
    ...
    load_base_data(index, ann_test_name, index_key, res, metric_type, dim, index_add_loops);
...
int main(void) {
    std::vector<size_t> param_nprobes = {64};
    const int32_t SEARCH_LOOPS = 5;

    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    const int32_t SIFT_INSERT_LOOPS = 1;  // insert twice to get ~1G data set
